...
// NOTE (Daniela, Aug 2026): the SUPPLIER block is intentionally REMOVED from every document —
// no PO or packing slip may ever show the supplier's details (so paperwork that reaches the end
// customer never reveals who we sourced from). Layout is FROM (TSO) | DELIVER TO (customer) only.
function fromDel(del){
 return [new Table({width:{size:9700,type:WidthType.DXA},columnWidths:[4850,4850],borders:{top:thin,bottom:thin,left:thin,right:thin,insideHorizontal:thin,insideVertical:thin},rows:[new TableRow({children:[
  cell(block('FROM',['Trade Superstore Online (TSO UK Ltd)','Unit 8, Tomlinson Industrial Estate','Alfreton Road, Derby, DE21 4ED','T: 0333 090 9217','hello@tradesuperstoreonline.co.uk']),{w:4850,fill:PALE}),
  cell(block('DELIVER TO',del),{w:4850})]})]}), P(T('',{size:6}),{after:40})];
}
...
function poDoc(d){
 const c=[...header('PURCHASE ORDER', d.po), ...fromDel(d.dl), ...metaRow(d.acct,d.order,d.req,d.contact)];
...
function slipDoc(d){
 ...
 const c=[...header('PACKING SLIP', d.po), ...fromDel(del), ...metaRow(acct, d.order, d.tso?'Next working day':'Standard', d.contact || 'TSO - 0333 090 9217')];
...
function validateDoc(d, kind, errors){
 ...
 // The SUPPLIER block was removed from every document by design (never show the supplier's
 // details), so there is deliberately no supplier-block validation here.
