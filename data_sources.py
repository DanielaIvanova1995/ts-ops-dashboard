...
def read_invoice_pdf(pdf_url: str) -> dict:
    ...
    prompt = (
        ...
        "qty is the LINE QUANTITY — how many of that line were ordered/supplied (the 'Qty' column). "
        "Some invoices (e.g. C Tie) ALSO show a 'Units' / pack-size / 'Units per pack' column, e.g. "
        "a 'Box of 100' line with Units 1000 but Qty 1 — in that case qty is 1 (the Qty column), "
        "NEVER the Units/pack count. If only one quantity column exists, use it. "
...
_INVOICE_HEADER_SYSTEM = (
    ...
    "them to negative. The po_number field is OUR purchase-order / order reference printed on the "
    "document — it is normally a 4-to-6 digit number (e.g. 30807, 28573). It may be labelled in many "
    "ways: \"PO\", \"P/O\", \"PO#\", \"PO Number\", \"Order\", \"Order No\", \"Your Order\", \"Your "
    "Ref\", \"Customer Ref\", \"Cust Ref\", \"Customer Order\", \"Customer Purchase Order\", "
    "\"Customer purchase order N°\", \"Customer PO\", \"Customer Reference\", \"Reference\", "
    "\"Ref\", or \"#\". Return ONLY the reference value itself, stripped of the label/prefix — e.g. "
    "\"Customer purchase order N°: 30666\" becomes \"30666\", \"Customer Ref 30807\" becomes "
    "\"30807\", \"PO26222\" becomes \"26222\", \"P/O 26226-1\" becomes "
    "\"26226-1\". Do NOT return the supplier's own invoice number, account code, or VAT number as the "
    "po_number. If there is genuinely no such order/customer reference, set po_number to null. "
    "Schema: {\"document_type\": string, \"supplier_name\": "
...
_TRIAGE_SYSTEM = (
    ...
    "— INCLUDING an auto-generated supplier despatch/delivery notification that names a specific "
    "order number AND a delivery date (e.g. Molan's \"Your order 1748566 will be delivered on "
    "21/09/2026\", or \"Your order N is out for delivery / arriving on <date>\"). These are "
    "supplier_with_eta, NEVER automated_system — the delivery date is important information we need\n"
    ...
    "enquiries, AND an inbound QUOTE/PRICE FROM ONE OF OUR SUPPLIERS (a supplier we buy from — e.g. "
    "GAP, Markovitz, Squaredeal, Molan, UPB, National Plastics — giving US a price for goods we asked "
    "them to price or source). A supplier quoting US is a SUPPLIER email, NOT a customer quote\n"
    ...
    "- customer_new_order_or_quote: A CUSTOMER (a buyer of ours, NOT one of our suppliers) enquiring "
    "about an UNPLACED order — asking US for a quote/price, product availability, product specs, "
    "discount codes, sample requests, pre-purchase queries, confirming intent to proceed with a new "
    "order, OR declining to place an order. A quote is only this category when a CUSTOMER is asking "
    "US for a price — if one of OUR SUPPLIERS is quoting US, that is supplier_no_eta, never this\n"
    ...
    "automated. A supplier email of the form \"Your order <number> will be delivered on <date>\" (auto-"
    "generated, one per order, e.g. from Molan) names a specific order and a delivery date → it is "
    "ALWAYS supplier_with_eta, never automated_system.\n"
    "2b. SUPPLIER QUOTE vs CUSTOMER QUOTE — a QUOTE is customer_new_order_or_quote ONLY when a "
    "CUSTOMER is asking US for a price. When one of OUR SUPPLIERS (GAP, Markovitz, Squaredeal, Molan, "
    "UPB, National Plastics, and any other trade supplier we buy from) sends US a quote/price for "
    "goods we asked them to price or source, they are the SELLER quoting the BUYER — that is "
    "supplier_no_eta, NEVER customer_new_order_or_quote. Tells: the sender is a trade-supplier domain/"
    "name, the email gives prices/quote refs TO us, or replies to our request for pricing. A supplier "
    "quote must never land in the customer quote queue.\n"
...
# ---------------------------------------------------------------------------
# Supplier STATEMENT reconciliation — read a statement PDF (any layout) with
...
def read_statement_pdf(pdf_url: str = None, pdf_bytes: bytes = None, text: str = None) -> dict:
    ...
    prompt = (
        ...
        "Reply with ONLY a JSON object, no other text:\n"
        '{"supplier":"the supplier/company that issued the statement",'
        '"customer_ref":"our account/customer number on the statement, or null",'
        '"statement_date":"YYYY-MM-DD","currency":"GBP",'
        '"balance":<the FINAL total the statement says we owe right now — the figure printed as '
        '"Balance Outstanding" / "Balance Due" / "Total Outstanding" / "Amount Due" / the closing '
        ...
        '"order_ref":"OUR order or reference number for this line if shown (e.g. a 5-digit order), '
        'else null",'
        '"amount":<line total, number; NEGATIVE for credits and payments>,'
...
def qbo_find_vendor(name: str):
    """Best-match QuickBooks Vendor for a supplier name → {id, name, balance}. None if not found.
    Tries an exact-ish name match, then a contains match on the statement supplier."""
    n = (name or "").strip().replace("'", "''")
    if not n:
        return None
    rows = qbo_query(f"select Id, DisplayName, Balance from Vendor where Active = true")
    vendors = rows.get("Vendor") or []
    low = name.strip().lower()
    # exact, then startswith, then token-overlap
    def score(v):
        d = (v.get("DisplayName") or "").lower()
        if d == low:
            return 3
        if d.startswith(low[:6]) or low.startswith(d[:6]):
            return 2
        aw, bw = set(low.split()), set(d.split())
        return 1 if (aw & bw) else 0
    vendors = [v for v in vendors if score(v) > 0]
    if not vendors:
        return None
    best = max(vendors, key=score)
    return {"id": best.get("Id"), "name": best.get("DisplayName"),
            "balance": best.get("Balance")}


def qbo_vendor_bills(vendor_id: str, limit: int = 1000):
...
def op_set_branch(item_id, branch=None, email=None, phone=None, token: str | None = None):
    ...
    if branch is not None:
        q = ("mutation($b:ID!,$i:ID!,$c:String!,$v:String!){change_simple_column_value("
             "board_id:$b,item_id:$i,column_id:$c,value:$v){id}}")
        r = requests.post(MONDAY_API, json={"query": q, "variables": {
            "b": str(ORDERS_BOARD_ID), "i": str(item_id), "c": OP_COLS["branch"],
            "v": str(branch)}}, headers=hdr, timeout=30)
        r.raise_for_status()
    if email is not None:
        val = {} if not email else {"email": email, "text": email}
...
def op_save_suggestion(who: str, msg: str, token: str | None = None) -> bool:
    """Save a team note / suggestion as a VISIBLE ROW on its own 'TradeHub Suggestions' board
    (named '[who] message'), so Daniela can scan them on a dedicated board — not buried in an
    item's Updates conversation. Full text also goes in the row's update in case the name is
    truncated. Returns True (raises if Monday rejects the create)."""
    token = token or get_token()
    board_id = monday_find_or_create_board(SUGGESTIONS_BOARD, token)
    who = (who or "team").strip()
    msg = msg.strip()
    item_id = monday_create_item(board_id, f"[{who}] {msg}"[:250], token)
    try:
...
def op_load_suggestions(limit: int = 8, token: str | None = None) -> list:
    ...
    return [{"body": i.get("name") or "", "created_at": i.get("created_at") or ""}
